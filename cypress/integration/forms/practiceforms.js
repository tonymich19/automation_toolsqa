...
describe('testing practice forms from ToolsQA', () => {
    ...
    it('Verify that radio but selected is loaded when checked', function() {
        ...
        cy.get('tbody > :nth-child(1) > :nth-child(2)').should('have.text', `${this.example.name} ${this.example.lastname}`)
        cy.get('tbody > :nth-child(2) > :nth-child(2)').should('have.text', this.example.email)
        cy.get('tbody > :nth-child(3) > :nth-child(2)').should('have.text', 'Male')
        cy.get('tbody > :nth-child(4) > :nth-child(2)').should('have.text', this.example.mobileNumber)
        ...
        cy.get('tbody > :nth-child(8) > :nth-child(2)').should('have.text', cvFile)
        cy.get('tbody > :nth-child(9) > :nth-child(2)').should('have.text', this.example.currentAdress)
